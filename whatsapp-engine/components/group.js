...
router.post('/sendmessage/:groupid', async (req, res) => {
    let groupid = req.params.groupid;
    let message = req.body.message;

    if (groupid == undefined || message == undefined) {
        res.send({ status: "error", message: "please enter valid chatname and message" })
    } else {
        client.getChats().then((data) => {
            data.forEach(chat => {
                if (chat.id.server === "g.us" && chat.id.user === groupid) {
                    client.sendMessage(chat.id._serialized, message).then((response) => {
                        if (response.id.fromMe) {
                            res.send({ status: 'success', message: `Message successfully send to ${groupid}`, detail: message })
                        }
                    });
                }
            });
